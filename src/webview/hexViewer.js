...
window.addEventListener('message', (e) => {
    ...
    switch (msg.type) {
        case 'init':
            state_1.S.parseResult = msg.parseResult;
            state_1.S.labels = msg.labels ?? [];
            state_1.S.rawSource = msg.rawSource ?? '';
            state_1.S.structs = msg.structs ?? [];
            state_1.S.structPins = msg.structPins ?? [];
            (0, data_1.initFlatBytes)();
            (0, data_1.buildMemRows)();
            // Choose default view: raw if there are errors, memory if valid
            const pr = state_1.S.parseResult;
            const fileOk = pr && pr.checksumErrors === 0 && pr.malformedLines === 0;
            state_1.S.currentView = fileOk ? 'memory' : 'raw';
            render();
            break;
        case 'addLabel':
            state_1.S.labels = [...state_1.S.labels, msg.label];
            (0, data_1.buildMemRows)();
            ...
            break;
        case 'updateLabel': {
            const updated = msg.label;
            state_1.S.labels = state_1.S.labels.map(l => l.id === updated.id ? updated : l);
            (0, data_1.buildMemRows)();
            render_1.rerender.labels();
            if (state_1.S.currentView === 'memory') {
                render_1.rerender.memory();
        ...
        case 'externalChange': {
            ...
            const incoming = {
                ...
                labels: msg.labels ?? [],
                rawSource: msg.rawSource ?? '',
            };
            if (state_1.S.editMode && state_1.S.edits.size > 0) {
...
function render() {
    document.getElementById('app').innerHTML = `
        <div id="toolbar">
            <div class="view-tabs">
                <button id="btn-raw" class="${state_1.S.currentView === 'raw' ? 'active' : ''}">Raw</button>
                <button id="btn-rec" class="${state_1.S.currentView === 'record' ? 'active' : ''}">Records</button>
                <button id="btn-mem" class="${state_1.S.currentView === 'memory' ? 'active' : ''}">Memory</button>
            </div>
            ...
            <div id="search-box">
                <select id="search-mode">
                    <option value="hex"   ${state_1.S.searchMode === 'hex' ? 'selected' : ''}>Hex</option>
                    <option value="ascii" ${state_1.S.searchMode === 'ascii' ? 'selected' : ''}>ASCII</option>
                    <option value="addr"  ${state_1.S.searchMode === 'addr' ? 'selected' : ''}>Addr</option>
                </select>
        ...
        <div id="main-area">
            <div id="content-pane">
                <div id="record-view" class="${state_1.S.currentView === 'record' ? 'visible' : ''}"></div>
                <div id="memory-view" class="${state_1.S.currentView === 'memory' ? 'visible' : ''}">
                    <div id="mem-header"></div>
                    <div id="mem-scroll"><div id="mem-rows"></div></div>
                </div>
                <div id="raw-view" class="${state_1.S.currentView === 'raw' ? 'visible' : ''}"></div>
            </div>
            <div id="sidebar">
                <div class="sb-tabs">
                    <button class="sb-tab ${state_1.S.sidebarTab === 'inspector' ? 'active' : ''}" id="sbt-insp">Inspector</button>
                    <button class="sb-tab ${state_1.S.sidebarTab === 'struct' ? 'active' : ''}" id="sbt-struct">Advanced</button>
                </div>
                <div class="sb-tab-panel ${state_1.S.sidebarTab === 'inspector' ? 'active' : ''}" id="sbp-insp">
                    <div class="sb-section" id="s-insp"></div>
                    <div class="sb-section" id="s-bits"></div>
                    <div class="sb-section" id="s-labels"></div>
                </div>
                <div class="sb-tab-panel ${state_1.S.sidebarTab === 'struct' ? 'active' : ''}" id="sbp-struct">
    ...
    document.getElementById('btn-cancel').addEventListener('click', () => {
        ...
        (0, sidebar_1.updateInspector)();
    });
    // Save button
    document.getElementById('btn-save').addEventListener('click', () => {
        if (state_1.S.edits.size === 0) {
            return;
        }
        api_1.vscode.postMessage({ type: 'saveEdits', edits: Array.from(state_1.S.edits.entries()) });
    });
    // Search
    const modeEl = document.getElementById('search-mode');
    const inputEl = document.getElementById('search-input');
    modeEl.addEventListener('change', () => { state_1.S.searchMode = modeEl.value; (0, search_1.runSearch)(); });
    inputEl.addEventListener('input', () => (0, search_1.runSearch)());
    inputEl.addEventListener('keydown', e => { if (e.key === 'Enter') {
        (e.shiftKey ? search_1.prevMatch : search_1.nextMatch)();
    } });
    document.getElementById('btn-prev').addEventListener('click', search_1.prevMatch);
    document.getElementById('btn-next').addEventListener('click', search_1.nextMatch);
    document.getElementById('btn-clear-search').addEventListener('click', search_1.clearSearch);
    ...
    document.addEventListener('keydown', e => {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === 'z' && state_1.S.editMode) {
            e.preventDefault();
            undoLastEdit();
        }
    });
    // Wire rerender callbacks
    render_1.rerender.memory = () => memRerender();
    render_1.rerender.labels = () => (0, sidebar_1.renderLabels)();
    render_1.rerender.toMemory = () => switchView('memory');
    // Wire search module
    (0, search_1.initSearch)(() => switchView('memory'));
    // Drag-to-select
    let dragAnchor = null;
    ...
    document.addEventListener('mousemove', e => {
        if (dragAnchor === null || !(e.buttons & 1)) {
            dragAnchor = null;
            return;
        }
        ...
        if (newStart === state_1.S.selStart && newEnd === state_1.S.selEnd) {
            return;
        }
        state_1.S.selStart = newStart;
        state_1.S.selEnd = newEnd;
        (0, memoryView_1.applySel)();
        (0, sidebar_1.updateInspector)();
    });
    ...
    // Initial renders
    renderStats();
    (0, memoryView_1.renderMemHeader)();
    (0, sidebar_1.renderInspector)();
    (0, sidebar_1.renderBits)();
    (0, struct_1.renderStructPanel)();
    (0, struct_1.renderStructPins)();
    (0, sidebar_1.renderLabels)();
    setupCtxMenu();
    if (state_1.S.currentView === 'memory') {
        memRerender();
    }
    else if (state_1.S.currentView === 'record') {
        renderRecordView();
    }
    else {
        renderRawView();
...
function renderStats() {
    ...
    if (!el || !state_1.S.parseResult) {
        return;
    }
    const p = state_1.S.parseResult;
    ...
    el.innerHTML =
        `<span class="si si-fmt"><span class="svl">${fmtLabel}</span></span>` +
            `<span class="si"><span class="slb">Bytes</span><span class="svl">${(0, utils_1.fmtB)(p.totalDataBytes)}</span></span>` +
...
// ── Memory view ───────────────────────────────────────────────────
function memRerender() {
    (0, memoryView_1.renderMemBody)(onByteDown, onByteCtx);
}
function onByteDown(e, el) {
    if (e.button !== 0) {
        return;
    } // ignore right/middle-click
    const addr = parseInt(el.dataset.addr, 16);
    ...
    if (e.shiftKey && state_1.S.selStart !== null) {
        if (addr < state_1.S.selStart) {
            state_1.S.selEnd = state_1.S.selStart;
    ...
    (0, memoryView_1.applySel)();
    (0, sidebar_1.updateInspector)();
    (0, struct_1.onSelectionChangeForStruct)();
}
function onByteCtx(e, el) {
    const addr = parseInt(el.dataset.addr, 16);
    if (!isNaN(addr)) {
        // Keep existing multi-byte selection if right-click is inside it;
        // otherwise collapse to the clicked byte.
        const inSel = state_1.S.selStart !== null && state_1.S.selEnd !== null
            && addr >= state_1.S.selStart && addr <= state_1.S.selEnd;
        if (!inSel) {
            state_1.S.selStart = addr;
            state_1.S.selEnd = addr;
            (0, memoryView_1.applySel)();
...
function selLen() {
    if (state_1.S.selStart === null || state_1.S.selEnd === null) {
        return 0;
    }
    return state_1.S.selEnd - state_1.S.selStart + 1;
}
// ── Record view ───────────────────────────────────────────────────
const IHEX_TYPE_LABELS = {
    0: 'DATA', 1: 'EOF', 2: 'EXT SEG ADDR', 3: 'START SEG ADDR',
    4: 'EXT LIN ADDR', 5: 'START LIN ADDR',
};
const SREC_TYPE_LABELS = {
...
function renderRecordView() {
    ...
    const isSrec = state_1.S.parseResult.format === 'srec';
    ...
    const rows = state_1.S.parseResult.records.map(r => {
        const isData = !r.error && (isSrec ? (r.recordType === 1 || r.recordType === 2 || r.recordType === 3)
            : r.recordType === 0);
        const badge = (!isSrec && (r.recordType === 4 || r.recordType === 2)) ? 'rb-ext' :
        ...
        const dataCell = r.error
            ? `<td class="rdata rerr-msg">${(0, utils_1.esc)(r.error)}</td>`
        ...
        return `<tr${gotoAttr}${rowClass}>
            ${addrCell}
            <td><span class="rbadge ${badge}">${(0, utils_1.esc)(lbl)}</span></td>
    ...
    el.querySelectorAll('tr[data-goto], td[data-goto]').forEach(cell => {
...
function renderRawView() {
    ...
    const isSrec = state_1.S.parseResult?.format === 'srec';
    const lines = state_1.S.rawSource.split(/\r?\n/);
    // Build a map: 1-based line number → parsed record (for error annotations)
    const recordByLine = new Map();
    if (state_1.S.parseResult) {
        for (const r of state_1.S.parseResult.records) {
            recordByLine.set(r.lineNumber, r);
        }
    }
    const rows = lines.map((line, i) => {
        ...
        if (isSrec) {
            return tokenizeSRecLine(ln, line, rec);
        }
        return tokenizeIHexLine(ln, line, rec);
    }).join('');
    // Build the errors panel
    const errorItems = state_1.S.parseResult ? state_1.S.parseResult.records
        .filter(r => r.error || !r.checksumValid)
        .map(r => {
        const icon = r.error ? 'err-icon-fmt' : 'err-icon-chk';
        const label = r.error ? r.error : `Checksum error: expected 0x${computeExpectedChecksum(r, state_1.S.parseResult.format).toString(16).toUpperCase().padStart(2, '0')}, got 0x${r.checksum.toString(16).toUpperCase().padStart(2, '0')}`;
        return `<div class="raw-err-item" data-line="${r.lineNumber}">` +
            ...
            `<span class="raw-err-msg">${(0, utils_1.esc)(label)}</span>` +
            `</div>`;
    }) : [];
    const panel = errorItems.length > 0
        ? `<div class="raw-problems"><div class="raw-problems-hdr">` +
            ...
            (state_1.S.parseResult.checksumErrors > 0
                ? `<button class="raw-repair-btn" id="btn-repair" title="Recompute and write correct checksums for all ${state_1.S.parseResult.checksumErrors} checksum error${state_1.S.parseResult.checksumErrors === 1 ? '' : 's'}">&#10003; Quick Repair</button>`
                : '') +
    ...
    el.querySelector('#btn-repair')?.addEventListener('click', () => {
        el.querySelector('#btn-repair').disabled = true;
        api_1.vscode.postMessage({ type: 'repairChecksums' });
    });
    el.querySelectorAll('.raw-err-item[data-line]').forEach(item => {
        item.addEventListener('click', () => {
            const lineNum = parseInt(item.dataset.line, 10);
            const target = el.querySelector(`.raw-line[data-ln="${lineNum}"]`);
...
function computeExpectedChecksum(r, format) {
    if (format === 'ihex') {
...
function tokenizeIHexLine(ln, line, rec) {
    const lineNum = ln.trim();
    if (!line.startsWith(':') || line.length < 11 || rec?.error) {
        const errTitle = rec?.error ? ` title="${(0, utils_1.esc)(rec.error)}"` : '';
        return `<div class="raw-line raw-malformed" data-ln="${lineNum}"${errTitle}>` +
            `<span class="raw-ln">${ln}</span><span class="raw-text">${(0, utils_1.esc)(line)}</span></div>`;
    }
    const ll = (0, utils_1.esc)(line.slice(1, 3));
    const addr = (0, utils_1.esc)(line.slice(3, 7));
    const tt = (0, utils_1.esc)(line.slice(7, 9));
    const type = parseInt(line.slice(7, 9), 16);
    const dataEnd = 9 + parseInt(line.slice(1, 3), 16) * 2;
    const data = (0, utils_1.esc)(line.slice(9, dataEnd));
    const chk = (0, utils_1.esc)(line.slice(dataEnd));
    const typeClass = type === 0 ? 'raw-t-data' :
        type === 1 ? 'raw-t-eof' :
            type === 4 ? 'raw-t-ela' :
...
function tokenizeSRecLine(ln, line, rec) {
    const lineNum = ln.trim();
    ...
    const bcHex = (0, utils_1.esc)(line.slice(2, 4));
    const addrHex = (0, utils_1.esc)(line.slice(4, 4 + aszChars));
    const byteCount = parseInt(line.slice(2, 4), 16);
    const dataEnd = 4 + aszChars + (byteCount - (aszChars / 2) - 1) * 2;
    const dataHex = (0, utils_1.esc)(line.slice(4 + aszChars, dataEnd));
    const chkHex = (0, utils_1.esc)(line.slice(dataEnd));
    const typeClass = (type === 1 || type === 2 || type === 3) ? 'raw-t-data' :
    ...
    return `<div class="raw-line" data-ln="${lineNum}">` +
        ...
        `<span class="raw-tt ${typeClass}">${(0, utils_1.esc)(typeChar)}</span>` +
...
function applyExternalChange(incoming) {
    state_1.S.parseResult = incoming.parseResult;
    state_1.S.labels = incoming.labels;
    state_1.S.rawSource = incoming.rawSource;
    (0, data_1.initFlatBytes)();
    (0, data_1.buildMemRows)();
    const ok = state_1.S.parseResult && state_1.S.parseResult.checksumErrors === 0 && state_1.S.parseResult.malformedLines === 0;
    state_1.S.currentView = ok ? 'memory' : 'raw';
    render();
    // Tell the provider it can update its own in-memory raw/parseResult
    api_1.vscode.postMessage({ type: 'reloadAccepted', rawSource: incoming.rawSource });
}
/** Show a non-destructive conflict banner when external change arrives during edit mode. */
function showExternalChangeConflict(incoming) {
    ...
    banner.innerHTML =
        `<span class="ecb-icon">⚠</span>` +
            `<span class="ecb-msg">File changed externally. You have <strong>${state_1.S.edits.size}</strong> unsaved edit${state_1.S.edits.size === 1 ? '' : 's'}.</span>` +
...
// ── Edit helpers ──────────────────────────────────────────────────
function applyFill(fillVal) {
    if (state_1.S.selStart === null || state_1.S.selEnd === null) {
        return;
    }
    const prev = [];
    for (let a = state_1.S.selStart; a <= state_1.S.selEnd; a++) {
        if (state_1.S.flatBytes.has(a)) {
            prev.push([a, state_1.S.flatBytes.get(a)]);
            state_1.S.flatBytes.set(a, fillVal);
            state_1.S.edits.set(a, fillVal);
        }
    }
    if (prev.length > 0) {
        state_1.S.undoStack.push(prev);
    }
    updateDirtyBar();
    if (state_1.S.currentView === 'memory') {
        memRerender();
...
function undoLastEdit() {
    if (!state_1.S.editMode || state_1.S.undoStack.length === 0) {
        return;
    }
    const txn = state_1.S.undoStack.pop();
    for (const [addr, prevVal] of txn) {
        state_1.S.flatBytes.set(addr, prevVal);
        const orig = getOriginalByte(addr);
        if (orig !== undefined && prevVal === orig) {
            state_1.S.edits.delete(addr);
        }
        else {
            state_1.S.edits.set(addr, prevVal);
        }
    }
    updateDirtyBar();
    if (state_1.S.currentView === 'memory') {
...
// ── Copy helpers ──────────────────────────────────────────────────
function getSelBytes() {
    if (state_1.S.selStart === null || state_1.S.selEnd === null) {
        return [];
    }
    const out = [];
    for (let a = state_1.S.selStart; a <= state_1.S.selEnd; a++) {
        out.push(state_1.S.flatBytes.get(a) ?? 0);
    }
    return out;
}
function handleCopyCommand(cmd) {
    ...
    switch (cmd) {
        case 'hex':
            text = bytes.map(h).join(' ');
            break;
        case 'hex-raw':
            text = bytes.map(h).join('');
            break;
        case 'binary':
            text = bytes.map(b => b.toString(2).padStart(8, '0')).join(' ');
            break;
        case 'ascii':
            text = bytes.map(b => (b >= 0x20 && b < 0x7F) ? String.fromCharCode(b) : '.').join('');
            break;
        case 'dec-array':
            text = `[${bytes.join(', ')}]`;
            break;
        case 'hex-array':
            text = `[${bytes.map(b => '0x' + h(b)).join(', ')}]`;
            break;
        case 'base64':
    ...
    api_1.vscode.postMessage({ type: 'copyText', text, label: `${bytes.length} bytes as ${cmd}` });
}
// ── CRC helpers ──────────────────────────────────────────────────
function crc8(data) {
    let c = 0;
...
function handleCtxCommand(cmd) {
    ...
    const h = (v, w = 2) => v.toString(16).toUpperCase().padStart(w, '0');
    // Copy
    ...
    if (cmd.startsWith('an-')) {
        ...
        switch (cmd) {
            ...
            case 'an-crc8':
                text = `0x${h(crc8(bytes))}`;
                label = 'CRC-8';
                break;
            case 'an-crc16':
                text = `0x${h(crc16(bytes), 4)}`;
                label = 'CRC-16';
                break;
            case 'an-crc32':
                text = `0x${h(crc32(bytes), 8)}`;
                label = 'CRC-32';
                break;
        }
        if (text) {
            api_1.vscode.postMessage({ type: 'copyText', text, label });
        }
        return;
    }
    // Fill / Patch — edit bytes in place (edit mode) or noop
    if (cmd.startsWith('fill-')) {
        if (!state_1.S.editMode) {
            return;
        }
...
function showCtxMenu(x, y) {
    if (state_1.S.selStart === null || selLen() === 0) {
        return;
    }
    ...
    const preview = (s) => s.length > 20 ? `${s.slice(0, 18)}…` : s;
    // Build helpers
    const item = (cmd, label, hint = '') => `<div class="ctx-row" data-cmd="${cmd}">` +
        `<span class="ctx-label">${(0, utils_1.esc)(label)}</span>` +
        (hint ? `<span class="ctx-hint">${(0, utils_1.esc)(hint)}</span>` : '') +
        `</div>`;
    const sep = `<div class="ctx-sep"></div>`;
    const sub = (label, id, body) => `<div class="ctx-row ctx-has-sub" data-sub="${id}">` +
        `<span class="ctx-label">${(0, utils_1.esc)(label)}</span>` +
    ...
    const customRow = `<div class="ctx-custom-row">` +
        ...
        `</div></div>`;
    const fillMenu = fillPresets.map(([v, lbl]) => item(`fill-${h(v)}`, lbl, len > 1 ? `× ${len}` : '')).join('') +
        sep +
        customRow;
    if (len === 1) {
        ...
        const copyMenu = item('hex', 'Hex', hexV) +
        ...
        menuBody =
            sub('Copy', 'copy', copyMenu) +
                (state_1.S.editMode ? sep + sub(fillLabel, 'fill', fillMenu) : '');
    }
    else {
        // ── Multi-byte: Copy + Analyze + Fill submenus ──
        const copyMenu = item('hex', 'Hex (spaces)', preview(bytes.map(b => h(b)).join(' '))) +
        ...
        const analyzeMenu = item('an-sum', 'Sum', `0x${h(sum, sumW)}  (${sum})`) +
    ...
    el.innerHTML =
        `<div class="ctx-hdr">${(0, utils_1.esc)(`${len} byte${len === 1 ? '' : 's'} selected`)}</div>` +
            (state_1.S.editMode ? `<div class="ctx-edit-badge">✏ Editing</div>` : '') +
            sep +
            menuBody;
    // Wire leaf-item clicks
    el.querySelectorAll('.ctx-row[data-cmd]').forEach(row => row.addEventListener('click', ev => {
        ev.stopPropagation();
        handleCtxCommand(row.dataset.cmd);
        hideCtx();
    }));
    // Wire custom fill input
    const fillInput = el.querySelector('.ctx-fill-input');
    const fillApply = el.querySelector('.ctx-fill-apply');
    ...
    fillInput?.addEventListener('focus', () => {
        ...
        const sub = fillInput.closest('.ctx-submenu');
        if (sub) {
...
function wireSubmenus(menuEl) {
    let closeTimer = null;
    let activeSub = null;
    const openSub = (sub, row) => {
        if (closeTimer) {
    ...
    menuEl.querySelectorAll('.ctx-has-sub').forEach(row => {
        const sub = row.querySelector('.ctx-submenu');
        if (!sub) {
            return;
        }
