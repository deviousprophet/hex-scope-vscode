...
exports.RECORD_TYPE_NAMES = void 0;
exports.parseIntelHex = parseIntelHex;
/** Display names for Intel HEX record types. */
exports.RECORD_TYPE_NAMES = {
    0x00: 'Data',
    0x01: 'End of File',
...
function parseLine(raw, lineNumber) {
    ...
    if (!(recordType in exports.RECORD_TYPE_NAMES)) {
    ...
    const REQUIRED_BYTE_COUNT = { 0x01: 0, 0x02: 2, 0x03: 4, 0x04: 2, 0x05: 4 };
    const requiredBC = REQUIRED_BYTE_COUNT[recordType];
    if (requiredBC !== undefined && byteCount !== requiredBC) {
        return { lineNumber, raw, byteCount, address, recordType, data, checksum, checksumValid, resolvedAddress: 0,
            error: `${exports.RECORD_TYPE_NAMES[recordType]} must have byte count ${requiredBC}, got ${byteCount}` };
    }
    return { lineNumber, raw, byteCount, address, recordType, data, checksum, checksumValid, resolvedAddress: 0 };
}
// ── Segment builder ───────────────────────────────────────────────
function buildSegments(records) {
    ...
    for (const rec of records) {
        if (rec.error || rec.recordType !== 0 /* RecordType.Data */ || !rec.checksumValid) {
            continue;
        }
        if (!current) {
...
//# sourceMappingURL=IntelHexParser.js.map