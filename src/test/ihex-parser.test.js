...
suite('IntelHexParser', () => {
    ...
    test('reads a well-formed data record', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)([rec, EOF].join('\n'));
        assert.strictEqual(result.records.length, 2);
        const r = result.records[0];
        assert.strictEqual(r.recordType, 0 /* RecordType.Data */);
    ...
    test('accepts the End of File record', () => {
        const result = (0, IntelHexParser_1.parseIntelHex)(EOF);
        assert.strictEqual(result.records.length, 1);
        assert.strictEqual(result.records[0].recordType, 1 /* RecordType.EndOfFile */);
        assert.strictEqual(result.records[0].checksumValid, true);
    });
    test('blank lines are silently skipped', () => {
        const src = `\n${dataRec(0x0000, [0x11])}\n\n${EOF}\n`;
        assert.strictEqual((0, IntelHexParser_1.parseIntelHex)(src).records.length, 2);
    });
    test('CRLF line endings are handled correctly', () => {
        const src = [dataRec(0x0000, [0x01]), dataRec(0x0001, [0x02]), EOF].join('\r\n');
        const result = (0, IntelHexParser_1.parseIntelHex)(src);
    ...
    test('corrupted checksum byte is flagged as invalid', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)(corrupted);
    ...
    test('checksumErrors counts every failing record', () => {
        ...
        assert.strictEqual((0, IntelHexParser_1.parseIntelHex)([bad1, bad2, EOF].join('\n')).checksumErrors, 2);
    });
    test('cleanly parsed file reports zero checksum errors', () => {
        const result = (0, IntelHexParser_1.parseIntelHex)([dataRec(0x0000, [0x01, 0x02, 0x03]), EOF].join('\n'));
        assert.strictEqual(result.checksumErrors, 0);
    });
    // ── Malformed records ─────────────────────────────────────────────────
    test('line not starting with ":" is recorded as malformed', () => {
        const result = (0, IntelHexParser_1.parseIntelHex)('0100000000FF');
    ...
    test('unknown record type is flagged as malformed', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)(':00000006FA');
        assert.strictEqual(result.malformedLines, 1);
    ...
    test('EOF record with non-zero byte count is flagged as malformed', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)(':02000001AABB98');
        assert.strictEqual(result.malformedLines, 1);
    ...
    test('Extended Linear Address with wrong byte count is flagged as malformed', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)(':0400000400080000F0');
        assert.strictEqual(result.malformedLines, 1);
        assert.ok(result.records[0].error?.includes('byte count'));
    });
    // ── Address resolution ────────────────────────────────────────────────
    test('without any address extension record the address is 16-bit', () => {
        const result = (0, IntelHexParser_1.parseIntelHex)([dataRec(0x1234, [0xFF]), EOF].join('\n'));
    ...
    test('Extended Linear Address record (type 04) sets the high 16 bits', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)(src);
        const dr = result.records.find(r => r.recordType === 0 /* RecordType.Data */);
    ...
    test('successive Extended Linear Address records each update the base', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)(src);
        const drs = result.records.filter(r => r.recordType === 0 /* RecordType.Data */);
    ...
    test('Extended Segment Address record (type 02) applies a 4-bit left shift', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)([type02, dataRec(0x0004, [0xAB]), EOF].join('\n'));
        const dr = result.records.find(r => r.recordType === 0 /* RecordType.Data */);
        assert.strictEqual(dr.resolvedAddress, 0x1000 + 0x0004);
    });
    test('Start Linear Address record (type 05) sets the program entry point', () => {
        const result = (0, IntelHexParser_1.parseIntelHex)(':04000005000800003F\n' + EOF);
    ...
    test('bad-checksum records are omitted from segments', () => {
        ...
        const result = (0, IntelHexParser_1.parseIntelHex)([bad, good, EOF].join('\n'));
        assert.strictEqual(result.segments.length, 1);
    ...
    test('totalDataBytes sums the lengths of all valid segments', () => {
        ...
        assert.strictEqual((0, IntelHexParser_1.parseIntelHex)(src).totalDataBytes, 5);
    });
    test('totalDataBytes is zero when the file contains only an EOF record', () => {
        assert.strictEqual((0, IntelHexParser_1.parseIntelHex)(EOF).totalDataBytes, 0);
    });
    test('empty string produces no records and no segments', () => {
        const result = (0, IntelHexParser_1.parseIntelHex)('');
    ...
    test('whitespace-only input produces no records', () => {
        assert.strictEqual((0, IntelHexParser_1.parseIntelHex)('   \n  \n').records.length, 0);
    });
});
//# sourceMappingURL=ihex-parser.test.js.map