...
const loadHex = (name) => fs.readFileSync(path.join(IHEX_DIR, name), 'utf8');
const loadSrec = (name) => fs.readFileSync(path.join(SREC_DIR, name), 'utf8');
// ── minimal.srec ─────────────────────────────────────────────────────────────
...
suite('sample/srec: minimal.srec', () => {
    let r;
    setup(() => { r = (0, SRecParser_1.parseSRec)(loadSrec('minimal.srec')); });
    ...
    test('all S1 records fit within 16-bit address space', () => {
        for (const rec of r.records.filter(rec => rec.recordType === 1)) {
            assert.ok(rec.resolvedAddress <= 0xFFFF, `address 0x${rec.resolvedAddress.toString(16)} exceeds 16-bit range`);
        }
    });
    test('all record checksums are valid', () => {
        for (const rec of r.records.filter(rec => !rec.error)) {
            assert.ok(rec.checksumValid, `checksum invalid on line ${rec.lineNumber}: ${rec.raw}`);
        }
    });
});
// ── firmware_s1.srec ──────────────────────────────────────────────────────────
...
suite('sample/srec: firmware_s1.srec', () => {
    let r;
    setup(() => { r = (0, SRecParser_1.parseSRec)(loadSrec('firmware_s1.srec')); });
    test('no parse errors', () => {
        assert.strictEqual(r.checksumErrors, 0);
        assert.strictEqual(r.malformedLines, 0);
    });
    ...
    test('uses only S1 data records', () => {
        ...
        assert.ok(dataRecs.every(rec => rec.recordType === 1), 'expected only S1 data records');
    });
    ...
    test('reset vector bytes 0x02,0x00,0x30 present at start of first segment', () => {
        assert.deepStrictEqual(Array.from(r.segments[0].data.slice(0, 3)), [0x02, 0x00, 0x30]);
    });
});
// ── firmware_s3.srec ──────────────────────────────────────────────────────────
...
suite('sample/srec: firmware_s3.srec', () => {
    let r;
    setup(() => { r = (0, SRecParser_1.parseSRec)(loadSrec('firmware_s3.srec')); });
    test('no parse errors', () => {
        assert.strictEqual(r.checksumErrors, 0);
        assert.strictEqual(r.malformedLines, 0);
    });
    ...
    test('all records use 32-bit addresses (fits in 4-byte S3 addr field)', () => {
        for (const rec of r.records.filter(rec => rec.recordType === 3)) {
            assert.ok(rec.resolvedAddress >= 0x08000000, `address 0x${rec.resolvedAddress.toString(16)} unexpectedly small`);
        }
    });
});
// ── stm32_s3.srec ─────────────────────────────────────────────────────────────
...
suite('sample/srec: stm32_s3.srec', () => {
    let r;
    setup(() => { r = (0, SRecParser_1.parseSRec)(loadSrec('stm32_s3.srec')); });
    test('no parse errors', () => {
        assert.strictEqual(r.checksumErrors, 0);
        assert.strictEqual(r.malformedLines, 0);
    });
    ...
    test('there is a large address gap between code and flash config', () => {
        ...
        assert.ok(flashSeg.startAddress > codeSeg.startAddress + codeSeg.data.length, 'expected address gap between code and flash config');
    });
...
suite('sample/srec: mixed_addr.srec', () => {
    let r;
    setup(() => { r = (0, SRecParser_1.parseSRec)(loadSrec('mixed_addr.srec')); });
    test('no parse errors', () => {
        assert.strictEqual(r.checksumErrors, 0);
        assert.strictEqual(r.malformedLines, 0);
    });
    ...
    test('S3 segment data contains 0xDEADBEEF 0xCAFEBABE', () => {
        const seg = r.segments.find(s => s.startAddress === 0x08000000);
        assert.deepStrictEqual(Array.from(seg.data), [0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE]);
    });
...
suite('sample/srec: errors.srec', () => {
    let r;
    setup(() => { r = (0, SRecParser_1.parseSRec)(loadSrec('errors.srec')); });
...
suite('sample/srec: cross-file edge cases', () => {
    test('S1 and IHEX files produce structurally equivalent segments when data is the same', () => {
        ...
        const sri = (0, SRecParser_1.parseSRec)(loadSrec('firmware_s1.srec'));
        const ihx = (0, IntelHexParser_1.parseIntelHex)(loadHex('minimal.hex'));
        assert.strictEqual(sri.totalDataBytes, ihx.totalDataBytes, 'SREC and IHEX should have same total bytes for equivalent content');
        assert.strictEqual(sri.segments.length, ihx.segments.length, 'SREC and IHEX should produce the same number of segments');
        for (let i = 0; i < sri.segments.length; i++) {
            assert.strictEqual(sri.segments[i].startAddress, ihx.segments[i].startAddress, `segment ${i} start address mismatch`);
            assert.deepStrictEqual(Array.from(sri.segments[i].data), Array.from(ihx.segments[i].data), `segment ${i} data mismatch`);
        }
    });
    test('mixed_addr: all three address width records parse without errors', () => {
        const r = (0, SRecParser_1.parseSRec)(loadSrec('mixed_addr.srec'));
        assert.strictEqual(r.checksumErrors, 0);
        assert.strictEqual(r.malformedLines, 0);
        assert.strictEqual(r.records.filter(rec => rec.error).length, 0);
    });
    test('errors.srec: no valid data from corrupt records', () => {
        const r = (0, SRecParser_1.parseSRec)(loadSrec('errors.srec'));
        ...
        for (const rec of corruptRecs) {
            const inSeg = r.segments.some(s => rec.resolvedAddress >= s.startAddress &&
                rec.resolvedAddress < s.startAddress + s.data.length);
            assert.ok(!inSeg, `corrupt record at 0x${rec.resolvedAddress.toString(16)} should not be in segments`);
        }
    });
    test('stm32_s3: start address from S7 record is reasonable (code region)', () => {
        const r = (0, SRecParser_1.parseSRec)(loadSrec('stm32_s3.srec'));
        assert.ok(r.startAddress !== undefined, 'startAddress should be set from S7 record');
        assert.ok(r.startAddress >= 0x08000000 && r.startAddress < 0x08020000, `start address 0x${r.startAddress.toString(16)} is outside expected STM32 flash range`);
    });
    test('firmware_s3: each segment occupies a distinct address range with no overlap', () => {
        const r = (0, SRecParser_1.parseSRec)(loadSrec('firmware_s3.srec'));
        const sorted = [...r.segments].sort((a, b) => a.startAddress - b.startAddress);
        for (let i = 1; i < sorted.length; i++) {
            const prev = sorted[i - 1];
            const curr = sorted[i];
            assert.ok(curr.startAddress >= prev.startAddress + prev.data.length, `segments ${i - 1} and ${i} overlap`);
        }
    });
    test('all SREC sample files parse without throwing', () => {
        ...
        for (const f of files) {
            assert.doesNotThrow(() => (0, SRecParser_1.parseSRec)(loadSrec(f)), `parseSRec threw on ${f}`);
        }
    });
});
//# sourceMappingURL=srec-samples.test.js.map