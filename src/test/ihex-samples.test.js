...
suite('sample: minimal.hex', () => {
    let r;
    setup(() => { r = (0, IntelHexParser_1.parseIntelHex)(loadHex('minimal.hex')); });
...
suite('sample: firmware.hex', () => {
    let r;
    setup(() => { r = (0, IntelHexParser_1.parseIntelHex)(loadHex('firmware.hex')); });
...
suite('sample: stm32_16bpr.hex', () => {
    let r;
    setup(() => { r = (0, IntelHexParser_1.parseIntelHex)(loadHex('stm32_16bpr.hex')); });
    test('no parse errors', () => {
        assert.strictEqual(r.checksumErrors, 0);
        assert.strictEqual(r.malformedLines, 0);
    });
    test('all data records use at most 16 bytes per row', () => {
        for (const rec of r.records.filter(rec => rec.recordType === 0x00)) {
            assert.ok(rec.byteCount <= 16, `unexpected byteCount ${rec.byteCount} at line ${rec.lineNumber}`);
        }
    });
    ...
    test('there is an address gap between the first and last segments', () => {
        ...
        assert.ok(last.startAddress > first.startAddress + first.data.length, 'expected a gap between first and last segment');
    });
});
// ── stm32_32bpr.hex ──────────────────────────────────────────────────────────
...
suite('sample: stm32_32bpr.hex', () => {
    ...
    let r16;
    setup(() => {
        r32 = (0, IntelHexParser_1.parseIntelHex)(loadHex('stm32_32bpr.hex'));
        r16 = (0, IntelHexParser_1.parseIntelHex)(loadHex('stm32_16bpr.hex'));
    });
    test('no parse errors', () => {
    ...
    test('segment data is byte-for-byte identical to the 16-bpr variant', () => {
        for (let i = 0; i < r32.segments.length; i++) {
            assert.strictEqual(r32.segments[i].startAddress, r16.segments[i].startAddress, `segment ${i}: start address mismatch`);
            assert.deepStrictEqual(Array.from(r32.segments[i].data), Array.from(r16.segments[i].data), `segment ${i}: data mismatch`);
        }
    });
});
// ── errors.hex ───────────────────────────────────────────────────────────────
...
suite('sample: errors.hex', () => {
    let r;
    setup(() => { r = (0, IntelHexParser_1.parseIntelHex)(loadHex('errors.hex')); });
...
//# sourceMappingURL=ihex-samples.test.js.map