...
suite('SRecParser', () => {
    ...
    test('parses a well-formed S1 data record', () => {
        ...
        const result = (0, SRecParser_1.parseSRec)(src);
    ...
    test('blank lines are silently skipped', () => {
        const src = `\n${s1Rec(0, [0x11])}\n\n${s9Rec()}\n`;
        assert.strictEqual((0, SRecParser_1.parseSRec)(src).records.length, 2);
    });
    ...
    test('detects a bad checksum', () => {
        ...
        const result = (0, SRecParser_1.parseSRec)([bad, s9Rec()].join('\n'));
    ...
    test('detects missing S start code', () => {
        const result = (0, SRecParser_1.parseSRec)(':00000001FF\n' + s9Rec());
    ...
    test('detects invalid record type character', () => {
        const result = (0, SRecParser_1.parseSRec)('SX0300001C\n' + s9Rec());
        assert.ok(result.records[0].error?.includes('Invalid record type character'));
        assert.strictEqual(result.malformedLines, 1);
    });
    test('detects non-hex characters', () => {
        const result = (0, SRecParser_1.parseSRec)('S10ZXXXX0102030405060708ZZ\n' + s9Rec());
    ...
    test('detects wrong byte count', () => {
        ...
        const result = (0, SRecParser_1.parseSRec)([truncated, s9Rec()].join('\n'));
        assert.ok(result.records[0].error);
        assert.strictEqual(result.malformedLines, 1);
    });
    test('reserved S4 record type is flagged as malformed', () => {
        // S4 with byteCount=3, addr=0x0000, chk=0xFC
        const result = (0, SRecParser_1.parseSRec)(['S4030000FC', s9Rec()].join('\n'));
        assert.strictEqual(result.malformedLines, 1);
    ...
    test('byte count too small for record type is flagged as malformed', () => {
        ...
        const result = (0, SRecParser_1.parseSRec)(['S10200FF', s9Rec()].join('\n'));
        assert.strictEqual(result.malformedLines, 1);
    ...
    test('S9 end record with a data payload is flagged as malformed', () => {
        ...
        const result = (0, SRecParser_1.parseSRec)(['S9050000AABB95', s9Rec()].join('\n'));
        assert.strictEqual(result.malformedLines, 1);
    ...
    test('counts total data bytes correctly', () => {
        ...
        assert.strictEqual((0, SRecParser_1.parseSRec)(src).totalDataBytes, 5);
    });
    test('counts multiple checksum errors', () => {
        const bad = (rec) => rec.slice(0, -2) + '00';
        const src = [
            bad(s1Rec(0x0000, [0xAA])),
            bad(s1Rec(0x0010, [0xBB])),
            s9Rec(),
        ].join('\n');
        assert.strictEqual((0, SRecParser_1.parseSRec)(src).checksumErrors, 2);
    });
    ...
    test('srecIsData returns true only for types 1, 2, 3', () => {
        assert.strictEqual((0, SRecParser_1.srecIsData)(0), false);
        assert.strictEqual((0, SRecParser_1.srecIsData)(1), true);
        assert.strictEqual((0, SRecParser_1.srecIsData)(2), true);
        assert.strictEqual((0, SRecParser_1.srecIsData)(3), true);
        assert.strictEqual((0, SRecParser_1.srecIsData)(5), false);
        assert.strictEqual((0, SRecParser_1.srecIsData)(7), false);
        assert.strictEqual((0, SRecParser_1.srecIsData)(9), false);
    });
    test('SREC_ADDR_SIZES has correct values', () => {
        assert.strictEqual(SRecParser_1.SREC_ADDR_SIZES[0], 2);
        assert.strictEqual(SRecParser_1.SREC_ADDR_SIZES[1], 2);
        assert.strictEqual(SRecParser_1.SREC_ADDR_SIZES[2], 3);
        assert.strictEqual(SRecParser_1.SREC_ADDR_SIZES[3], 4);
        assert.strictEqual(SRecParser_1.SREC_ADDR_SIZES[7], 4);
        assert.strictEqual(SRecParser_1.SREC_ADDR_SIZES[8], 3);
        assert.strictEqual(SRecParser_1.SREC_ADDR_SIZES[9], 2);
    });
    // ── Edge cases ────────────────────────────────────────────────────────
    ...
    test('empty source returns empty parse result', () => {
        const result = (0, SRecParser_1.parseSRec)('');
        assert.strictEqual(result.records.length, 0);
        assert.strictEqual(result.segments.length, 0);
        assert.strictEqual(result.totalDataBytes, 0);
    });
});
//# sourceMappingURL=srec-parser.test.js.map