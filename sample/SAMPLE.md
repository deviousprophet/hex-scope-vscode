...
```
sample/
â”œâ”€â”€ ihex/          Intel HEX (.hex) samples
â”‚   â”œâ”€â”€ minimal.hex
â”‚   â”œâ”€â”€ firmware.hex
â”‚   â”œâ”€â”€ stm32_16bpr.hex
â”‚   â”œâ”€â”€ stm32_32bpr.hex
â”‚   â””â”€â”€ errors.hex
â””â”€â”€ srec/          Motorola SREC (.srec) samples
    â”œâ”€â”€ minimal.srec
    â”œâ”€â”€ firmware_s1.srec
    â”œâ”€â”€ firmware_s3.srec
    â”œâ”€â”€ stm32_s3.srec
    â”œâ”€â”€ mixed_addr.srec
    â””â”€â”€ errors.srec
```
...
**Stats:** 2 segments Â· 40 bytes Â· 0 errors  
**Use for:** Simplest valid IHEX file; no ELA records; tests basic 16-bit parsing and contiguous segment building.
...
| Address | Content |
|---------|---------|
| `0x08000000` | Cortex-M3 partial vector table (2 Ã— 16-byte records) |
| `0x08002000` | `0xDEADBEEF` + `0xCAFEBABE` magic + sequential data |
| `0x08010000` | `"HELLO WORLD!"` + bootloader string |

**Stats:** 3 segments Â· 9 records Â· 0 errors  
**Use for:** ELA segment switching, non-contiguous segments, short (14-byte) record, `0xDEADBEEF` sentinel detection.

---

### `stm32_16bpr.hex`
**Format:** Intel HEX I32, **16 bytes per data record** (`:10â€¦`)  
**Target:** STM32F103C8T6 (Cortex-M3, 64 KB flash)  
**Content:**
...
| `0x08000000` | Cortex-M3 vector table (SP + 19 exception/IRQ vectors) | 128 B |
| `0x080000C0` | ARM Thumb-2 Reset_Handler, SystemInit, main, app_run stubs | 70 B |
| `0x08010000` | Flash config block (magic, version, app_start, CRC) + string constants | 64 B |

**Stats:** 2 segments Â· 262 bytes Â· 0 errors  
**Use for:** Standard 16-byte record parsing, ELA-based segment gap, realistic STM32 layout.

---

### `stm32_32bpr.hex`
**Format:** Intel HEX I32, **32 bytes per data record** (`:20â€¦`)  
**Target:** Same as `stm32_16bpr.hex`  
**Content:** Byte-for-byte identical to `stm32_16bpr.hex`; only record size differs.

**Stats:** 2 segments Â· 262 bytes Â· 0 errors  
**Use for:** Verifying parser output is record-size-independent. Both files must yield identical segments, addresses, and byte counts.
...
### `errors.hex`
**Format:** Intel HEX I32 with intentional errors  
**Content:** Valid records interspersed with 2 bad checksums and 1 malformed (non-`:`) line.

**Stats:** invalid Â· 2 checksum errors Â· 1 malformed line  
**Use for:** Error detection, partial segment building (only valid records contribute bytes).
...
### `minimal.srec`
**Format:** SREC with S1 (2-byte address) data records  
**Content:** S0 header Â· 2 Ã— S1 data Â· S5 record count Â· S9 end-of-file  

| Address | Content | Bytes |
|---------|---------|-------|
| `0x0000` | Sequential bytes `0x01â€“0x10` | 16 |

**Stats:** 1 segment Â· 16 bytes Â· 5 records Â· 0 errors  
**Use for:** Simplest valid SREC file; exercises S0, S1, S5, S9 parsing and checksum validation.

---

### `firmware_s1.srec`
**Format:** SREC with S1 (2-byte address) records  
**Target:** 8051 microcontroller â€” content mirrors `ihex/minimal.hex`  
**Content:**
...
**Stats:** 2 segments Â· 40 bytes Â· 7 records Â· 0 errors Â· start `0x0030`  
**Use for:** Confirming SREC and IHEX parsers produce byte-identical segments for equivalent content. Cross-format parity test.
...
**Stats:** 3 segments Â· 103 bytes Â· 9 records Â· 0 errors Â· start `0x08000141`  
**Use for:** S3 32-bit address parsing, ELA-equivalent multi-segment layout, `0xDEADBEEF` sentinel, S7 execution address.
...
| `0x08000000` | Cortex-M3 vector table (SP + 19 vectors) | 128 |
| `0x080000C0` | ARM Thumb-2 Reset_Handler + app stubs | 70 |
| `0x08010000` | Flash config + identity strings | 66 |

**Stats:** 5 segments Â· 264 bytes Â· 20 records Â· 0 errors Â· start `0x080000C1`  
**Use for:** Realistic STM32 firmware in S3 format, large address gaps, S7 execution start.

---

### `mixed_addr.srec`
**Format:** SREC mixing **S1, S2, and S3** data records  
**Content:**

| Record | Address | Data |
|--------|---------|------|
| S1 | `0x0100` | `0x11 0x22 0x33 0x44` |
| S2 | `0x100000` | `0xAA 0xBB 0xCC 0xDD` |
| S3 | `0x08000000` | `0xDEADBEEF 0xCAFEBABE` (8 bytes) |

**Stats:** 3 segments Â· 16 bytes Â· 6 records Â· 0 errors  
**Use for:** Validating that the parser correctly resolves all three SREC address widths (2, 3, 4 bytes) within the same file.

---

### `errors.srec`
**Format:** SREC with intentional errors  
**Content:** S0 header Â· 2 S1 records with corrupt checksums Â· 1 non-SREC line Â· 1 valid S1 record Â· S9 end

**Stats:** 1 segment Â· 3 bytes Â· 2 checksum errors Â· 1 malformed line  
